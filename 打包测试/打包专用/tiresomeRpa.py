...
log_file = glob2.glob('..\\Software_file\\' + '\\*.txt')[0]
LogMethod = 2
sheet1 = None
nrows = 2
WorkPath = ''
...
def Readcmd():
    ...
    while NowRow < sheet1.nrows:  # 为了保证读全，nrows+1  # 不用加，nrows比nowrow大1，正好等于的时候停止了
...
def AllRowRun(commands):  # 遍历一次表  # n个commands，n+1行表格，nrows=n，i从0开始，共n个，最后是n-1
    ...
    while i < nrows-1:  # 其实这里commands[0]是表格里的第二行，commands行数比表格计数少1，nrows=sheet1.nrows比表格计行多一
        if commands[i].redo.value < 0:  # 先看是否执行redo 如果为-1则无限执行
            ...
            j = 0
            while j < commands[i].redo.value:  # 执行redo次，默认为1
                key = commands[i].work()
